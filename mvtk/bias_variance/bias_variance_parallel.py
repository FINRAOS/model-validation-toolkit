...
def bias_variance_compute_parallel(estimator, X_train, y_train, X_test, y_test,
    ...
    if random_state is None:
        ...
        result = [train_and_predict_ray.remote(estimator,
...
def train_and_predict_ray(estimator, X_train_values, y_train_values, X_test_prepared,
    ...
    Args:
        ...
        X_test_prepared: features for testing which has been processed by prepare_X function
        fit_kwargs (dict, optional): kwargs to pass to the fit method
        predict_kwargs (dict, optional): kwargs to pass to the predict method
...
@ray.remote
def bootstrap_train_and_predict_ray(estimator, X_train_values, y_train_values, X_test_prepared,
                          prepare_X=lambda x: x, prepare_y_train=lambda x: x,
                          fit_kwargs=None, predict_kwargs=None):
