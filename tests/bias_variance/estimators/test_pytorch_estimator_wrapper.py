...
def test_pytorch_estimator_wrapper_custom_fit():
    ...
    torch.manual_seed(123)
    model_test, optimizer_test, loss_fn_test = create_model()
    model_wrapped = PyTorchEstimatorWrapper(
        model_test, optimizer_gen, loss_fn_test, fit_fn=custom_fit
    )
...
def test_pytorch_estimator_wrapper_custom_predict():
    ...
    model_test, optimizer_test, loss_fn_test = create_model()
    model_wrapped = PyTorchEstimatorWrapper(
        model_test, optimizer_gen, loss_fn_test, predict_fn=custom_predict
    )
